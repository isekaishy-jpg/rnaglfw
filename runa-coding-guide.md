...
- `../RunaLang/spec/sema/io-and-files.md`
...
Use the toolchain scaffold commands first. Do not hand-roll directory trees
when `runa new` already owns the shape.
...
Rules from the current package-command spec:

- `runa new <name>` creates a bin package by default.
- The default package scaffold writes `runa.toml`.
- The default package scaffold writes `main.rna`.
- The default package scaffold writes one short standardized file-comment
  header block in `main.rna`.
- `runa new --lib <name>` creates a lib package scaffold.
- The library scaffold writes `lib.rna`.
- The library scaffold writes `core/mod.rna`.
- The library scaffold writes one short standardized file-comment header block
  in `lib.rna`.
- The library scaffold writes one short standardized file-comment header block
  in `core/mod.rna`.

Module creation inside an existing package:

- `runa new --mod=player`
- `runa new --mod=player.render`
- `runa new --mod=game.player --mod=game.assets`

Rules:

- `runa new --mod=<path>` requires one discovered target package.
- Each `--mod=<path>` is one dot-separated child-module path relative to the
  target package root.
- Module creation updates required parent `mod name` declarations
  deterministically.
- Module creation creates missing intermediate child-module directories and
  `mod.rna` entry files as needed.
- Module creation writes one standardized scaffold file-comment header block in
  each new `mod.rna`.
- Module creation is all-or-nothing for the whole invocation.
- Duplicate `--mod=<path>` values are hard errors.
- Module creation must reject overwrite of an existing module entry file.
- Module creation must reject incompatible existing file or directory layout.
- Module creation must not guess `pub` versus private visibility for new module
  declarations.

Standardized scaffold header expectations:

- The scaffold header begins with `Spec scaffold for ...`.
- The header includes:
  - `Spec refs:`
  - `Structure ownership:`
  - `Graph role:`
...
- For new package roots, prefer `runa new`.
- For new child-module paths inside this repo, prefer `runa new --mod=...`.
- From the workspace root, package creation targets new workspace members.
- For module creation in this repo, run `runa new --mod=...` from inside the
  target package directory such as `framework/` or `demo/`.
- Hand-author module trees only when intentionally repairing or reshaping an
  existing scaffold.
...
use std.io.{ByteSource, ByteSink, Flushable, file_open}
...
`rnaplayer` mostly uses the self-backed standard families, but the language has
a larger memory model than the old guide implied.
...
## Streams, Hosted IO, And Job Lanes

Runa now has one real `std.io` subsystem instead of only text and bytes held in
memory.

Generic stream rules:

- `std.io` owns `Source[T]` and `Sink[T]` for item-oriented streams.
- `std.io` also owns `ByteSource` and `ByteSink` for byte-specialized
  transport.
- Stream traits carry associated error families with `type Error`.
- `Option.None` from `next()` or `read_chunk()` means end of stream, not one
  error.
- Files and standard streams are hosted byte-transport implementations; they do
  not define all streaming in the language.
- `Flushable` and `Closable` are separate capabilities.
- Do not assume every stream is duplex, flushable, or closable.
- Do not hardwire user-defined stream families to `IoError`; `IoError` is the
  hosted `File` and stdio error family.

Hosted byte-I/O rules:

- Use `file_open(path, spec)` for explicit hosted file access.
- `FileOpenSpec` uses typed policy values:
  - `FileAccess`
  - `FileCreateMode`
  - `FileWriteMode`
- `stdin()`, `stdout()`, and `stderr()` return typed standard-stream handles.
- `write_all` is full-sequence-or-error; it does not surface partial-count
  success in v1.
- `read_chunk(limit)` may return fewer bytes than `limit`.
- `read_chunk(0)` is valid and does not advance source state.
- `close(take self)` is explicit and consuming.

Hosted I/O example:

```runa
use std.io.{ByteSink, Flushable, FileAccess, FileCreateMode, FileOpenSpec, FileWriteMode, file_open}

fn write_log(path: Str, read bytes: Bytes) -> Unit:
    let spec = FileOpenSpec :: :: call
        access = FileAccess.WriteOnly
        create = FileCreateMode.CreateAlways
        write_mode = FileWriteMode.Overwrite
    let-else Ok(file) = file_open :: path, spec :: call:
        return
    let _ = file.write_all :: bytes :: method
    let _ = file.flush :: :: method
    let _ = file.close :: :: method
```

Job-lane rules:

- `std.task` owns `Jobs[T, E, O]` for serialized shared-resource work.
- `T` is submitted job payload.
- `E` is explicit completion shape such as `Unit`, `Option[...]`, or
  `Result[...]`.
- `O` is ordering policy through `JobOrdering[T]`.
- `JobRunner[T, E]` owns actual work execution.
- `JobLaneSpec[O]` owns:
  - `ordering`
  - `capacity`
  - `when_full`
- Built-in ordering families include `SerialOrder` and `PriorityOrder`.
- `submit(...)` may suspend for admission.
- `try_submit(...)` never suspends for admission.
- `JobPlacement.NextJob` means next pending job after the current running job;
  it does not preempt a started job.
- Job lanes are non-preemptive in v1.
- Use job lanes for shared file/stream ownership, caches, or other serialized
  services; do not fake those semantics through ambient global state.

Job-lane example:

```runa
use std.task.{JobCapacity, JobFullPolicy, JobLaneSpec, JobRunner, Jobs, SerialOrder, jobs_local}
...
impl JobRunner[CounterJob, I32] for CounterRunner:
    suspend fn handle(edit self, take job: CounterJob) -> I32:
        select value job:
            when CounterJob.Add(delta) =>
                self.value = self.value + delta
                return self.value
        return self.value

suspend fn bump(read lane: Jobs[CounterJob, I32, SerialOrder]) -> I32:
    let task = lane.submit :: (CounterJob.Add :: 1 :: call) :: method
    let-else Ok(next) = task:
        return 0
    return next.await :: :: method

suspend fn run_counter() -> I32:
    let spec = JobLaneSpec :: :: call
        ordering = SerialOrder :: :: call
        capacity = JobCapacity.Unbounded
        when_full = JobFullPolicy.Wait
    let lane = jobs_local :: (CounterRunner :: :: call
        value = 0), spec :: call
    return bump :: lane :: call
```
...
`rnaplayer` is not as boundary-heavy as `rnaglfw`, but this is still general
language surface.
...
For `rnaplayer`, raw wrapper handles and app-layer handles should keep these
...
2. Check current direct dependencies for current package-level surfaces.
3. Do not use sibling app repos as API truth.
4. Validate with the configured Nightly `runa` on `PATH`.
5. If the spec is silent, contradictory, or still missing a needed rule, stop.
